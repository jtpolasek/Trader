...
export async function getUniswapQuote(params: UniswapQuoteParams) {
  ...
  const chainId = params.chainId ?? ETH_CHAIN_ID;
...
export function normalizeUniswapQuote(
  ...
  return {
    ...
    chainId: quote.chainId ?? params.chainId ?? ETH_CHAIN_ID,
...
export function assertUsableUniswapQuote(quote: NormalizedUniswapQuote) {
  ...
  const hasNoRoute = quote.warnings.some((warning) => warning.startsWith("No usable Uniswap route"));
...
export function summarizeUniswapIssues(rawResponse: Pick<UniswapRawQuote, "txFailureReason" | "detail" | "error">) {
  const warnings: string[] = [];
  const text = [rawResponse.txFailureReason, rawResponse.detail, rawResponse.error].filter(Boolean).join(" ");
...
function estimateDexFeeUsd(rawResponse: UniswapRawQuote) {
  const quote = rawResponse.quote;
  if (!quote?.portionAmount || !quote.portionBips) return 0;
  return 0;
}
